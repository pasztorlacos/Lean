...
class MyPositionManager:
    ...
    ENTER POSITION
    '''
    def EnterPosition(self, symbol, myDirection, myReverse=False):
        ...
        if direction == 1:
            ...
            riskperTrade = sd.var.longVaRLimit
            if debugEntry and riskperTrade < sd.CL.riskperLongTrade: 
                self.algo.MyDebug("     REDUCED RISK per TRADE from:{}%, to:{}% due to Startegy VaR LIMIT. Symbol: {}({}), TLongLimit:{}%, TLimit:{}%, StrOpenLVaR:{}%, StrOpenTVaR:{}%"
                    .format(str(round(sd.CL.riskperLongTrade*100,2)),
        ...
        if direction == -1:
            ...
            riskperTrade = sd.var.shortVaRLimit
            if debugEntry and riskperTrade < sd.CL.riskperShortTrade: 
                self.algo.MyDebug("     REDUCED RISK per TRADE from:{}%, to:{}% due to Str. VaR LIMIT. Symbol: {}({}), TShortLimit:{}%, TLimit:{}%, StrOpenSVaR:{}%, StrOpenTVaR:{}%"
                    .format(str(round(sd.CL.riskperShortTrade*100,2)),
                            str(round(riskperTrade*100,2)),
                            str(symbol),
                            str(strategyCode),
    ...
    '''
    def EnterPosition_2(self, symbol, myDirection):
        ...
        if pendingEntries['_SP']+pendingEntries['_SSubmitted']==0 and pendingEntries['_PFlip']==0:
            self.EnterPosition(symbol, myDirection, myReverse=False)
            return
        ...
        if (pendingEntries['_SP']+pendingEntries['_SSubmitted'])*myDirection < 0 and enableFlip:
            ...
            self.FlipPosition(symbol, myDirection)
            return
    ...
    def FlipPosition(self, symbol, myDirection):
        debugFlip = self.debug
        
        #Remove eny existing pending position for the symbol
        #This could be only opposite as this is the condition of calling FlipPosition
        ...
        self.CL.pendingFlipPositions.append((symbol, myDirection, expiryDate))
    ...
    def CheckPendingEntry(self):
        ...
        for pos in self.CL.pendingFlipPositions:
            ...
            if _SP+_SM == 0 and _ASS == 0 and _ASL == 0 and self.algo.mySymbolDict[symbol].entryEnabled:
                ...
                self.EnterPosition(symbol, direction, myReverse=False)
                #Remove Pendig Entry
                self.CL.pendingFlipPositions.remove(pos)
        return
