...
class MyZigZag():
    ...
    def FeatureExtractor(self, listLen=10, Type=1):
        ...
        features=[]
        if Type==1:
            #Normalized ATR distance of the zz point from current price 
...
class MyDCHState():
    ...
    def __init__(self, caller, baseDCH, referenceDCH=-1):
        self.debug = False 
...
class MyVolatility():
    def __init__(self, caller, algo, symbol, name, period, atr, benchmarkTicker=None):
        ...
        self.IsReady = False
        self.benchmarkTicker=benchmarkTicker
        self.benchmarkSymbol=None #Diring Initialize self.algo.Securities[self.benchmarkTicker].Symbol might not be available (initialized later)
