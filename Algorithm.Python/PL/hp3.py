...
class MyHelpers:
    ...
    # ["UPS", "USB", "UTX", "V", "VZ", "WBA", "WFC", "WMT", "XOM", "ATVI", "AMD", "ALXN", "ALGN", "AAL", "ADI", "AMAT", "ASML", "ADSK", "ADP", "BIDU", "BMRN", "AVGO", "CDNS", "CERN", "CHKP", "CTAS", "CTXS", "CTSH", "CSX", "CTRP", "DLTR", "EBAY", "EA", "EXPE", "FAST", "FISV", "FOX", "HAS", "HSIC", "IDXX", "ILMN", "INCY", "INTU", "ISRG", "JBHT", "JD", "KLAC", "LRCX", "LBTYA", "LBTYK", "LULU", "MAR", "MXIM", "MELI", "MCHP", "MU", "MNST", "MYL", "NTAP", "NTES", "NXPI", "ORLY", "PCAR", "PAYX", "REGN", "ROST", "SIRI", "SWKS", "SYMC", "SNPS", "TMUS", "TTWO", "TSLA", "ULTA", "UAL", "VRSN", "VRSK", "VRTX", "WDAY", "WDC", "WLTW", "WYNN", "XEL", "XLNX", "STX", "TSLA", "VRSK", "WYNN", "XLNX"]       
    # SP500-ES&NQ/100_1
    ...
    def MyOnWarmupFinished(self):
        ...
        if self.algo.LiveMode or False:
            ...
            if self.algo.updateSettings:
                ...
                self.algo.Schedule.On(self.algo.DateRules.EveryDay(), self.algo.TimeRules.Every(timedelta(minutes=6.1968)), Action(self.algo.strategySettings.UpdateSettings))
    ...
    def UnpackFeatures (self, features, featureType=1, myRegex='Feat', reshapeTuple=None, mySlicer=None):
        ...
        #Applying Regex and Slicing
        if mySlicer==None:
            df_filtered = df.filter(regex = myRegex)[:]
        else:
            df_filtered = df.filter(regex = myRegex)[mySlicer]
...
class MyStrategySettings():
    ...
    def UpdateSettings(self):
        ...
        #Update Strategies
        for strategy in self.algo.myStrategyClassList:
            if strategy.strategyCode in df:
                for row in range(df.shape[0]):
                    prop = df.index[row]
                    value = df.loc[df.index[row], strategy.strategyCode]
                    if hasattr(strategy, prop) and not pd.isna(value):
                        oldvalue = getattr(strategy, prop) 
                        if value!=oldvalue and ((isinstance(value, float) and isinstance(oldvalue, float)) or (isinstance(value, bool) and isinstance(oldvalue, bool))) and self.ValidateData(value, prop):
                            setattr(strategy, prop, value)
                            if  self.CL.debugChanges: self.algo.MyDebug(' ---- SETTING CHANGED!  {}.CL.{} = {}, oldvalue:{}, equal:{}'.format(strategy.strategyCode, prop, str(getattr(strategy, prop)), str(oldvalue), getattr(strategy, prop)==df.loc[df.index[row], strategy.strategyCode]))
                        if self.CL.debug: self.algo.MyDebug('{}.CL.{} value:{} csv_value:{}, equal:{}'.format(strategy.strategyCode, prop, str(getattr(strategy, prop)), df.loc[df.index[row], strategy.strategyCode], getattr(strategy, prop)==df.loc[df.index[row], strategy.strategyCode]))
        return
