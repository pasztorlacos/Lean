### <summary>-
### VaR Statistics v31 (VaR 3 was not synced by Skylight)
### Charting
### 
### </summary>
...
class MyVaR:
    ...
    def UpdateExposure (self):
        ...
        for pos in self.algo.Portfolio:
            ...
            if var.icnludeinTotalVaR and quantity>0:
                #Portfolio
        ...
        for symbol in self.algo.mySymbolDict:
            ...
            if var.icnludeinTotalVaR and portfolioValue!=0:
                if _SS < 0:
                    useTargets = getattr(self.algo.mySymbolDict[symbol].CL, "useTragetsLong", self.algo.mySymbolDict[symbol].CL.targetPlacerLong[0]!=None)
                else:
                    useTargets = getattr(self.algo.mySymbolDict[symbol].CL, "useTragetsShort", self.algo.mySymbolDict[symbol].CL.targetPlacerShort[0]!=None)
    ...
    def UpdateVaR (self):
        ...
        # x !=None condition due to live run exception AttributeError : 'NoneType' object has no attribute 'StopPrice' 
        for tempTicket in [x for x in self.algo.openStopMarketOrders if x !=None]:
            ...
            if var.icnludeinTotalVaR and portfolioValue !=0 and (tempTicket.Status == OrderStatus.Submitted or tempTicket.Status == OrderStatus.UpdateSubmitted or tempTicket.Status == 6 or tempTicket.Status == OrderStatus.New):
        ...
        for tempTicket in self.algo.openLimitOrders:
            ...
            if var.icnludeinTotalVaR and (tempTicket.Status == OrderStatus.Submitted or tempTicket.Status == OrderStatus.UpdateSubmitted or tempTicket.Status == 6):
    ...
    def PortfolioList(self, positiononly):
        #This is due to QC Refactoringh issue https://github.com/QuantConnect/Lean/commit/a697d7380e343d6ab710047e8f940d2e43ef9713
        if False:
            marginRemainingLong = round(self.algo.Portfolio.GetMarginRemaining(self.algo.benchmarkSymbol,OrderDirection.Buy))
            marginRemainingShort = round(self.algo.Portfolio.GetMarginRemaining(self.algo.benchmarkSymbol,OrderDirection.Sell))
            buyingPowerLong = round(self.algo.Portfolio.GetBuyingPower(self.algo.benchmarkSymbol,OrderDirection.Buy))
            buyingPowerShort = round(self.algo.Portfolio.GetBuyingPower(self.algo.benchmarkSymbol,OrderDirection.Sell))
        else:
            marginRemainingLong = 0
            marginRemainingShort = 0
            buyingPowerLong = 0
            buyingPowerShort =0
        ...
        if self.algo.LiveMode or False:
            ...
            self.algo.MyDebug("       Margin Remaining (Long/Short) for " + str(self.algo.benchmarkSymbol) + ": "+ str(marginRemainingLong)+ "/" + str(marginRemainingLong))
            self.algo.MyDebug("       Buying Power (Long/Short) for " + str(self.algo.benchmarkSymbol) + ": "+ str(buyingPowerLong)+ "/" + str(buyingPowerShort))
...
class MyStats:
    ...
    def PrintStrategyTradeStats(self):
        ...
        for strategy in self.algo.myStrategyClassList:
            i+=1
            self.algo.MyDebug("{} STAT for STRATEGY {}.: {}: Trades:{}, LongTrades:{}, ShortTrades:{}, FlipTrades:{}, EntryRejections:{}, Entries:{}, EntryFills:{}"
                   .format(str('\n' + str(' '*2)),
                        str(i),
