...
class MyHelpers:
    ...
    def UnpackFeatures (self, features, featureType=1, featureRegex='Feat', reshapeTuple=None, mySlicer=None):
        ...
        if mySlicer==None:
            if featureRegex==None or featureRegex=='Feat':
                df_filtered = df
            else:
                df_filtered = df.filter(regex = featureRegex)
        else:
            if featureRegex==None or featureRegex=='Feat':
                df_filtered = df[mySlicer]
            else:
...
class MyModelLoader:
    ...
    @classmethod
    def PrintFeatures(cls, caller, df_features, featureRegex='Feat', negativeOnly=False):
        if type(df_features) is list:
            caller.algo.MyDebug(f' features list: {df_features}')
            return
        df = df_features.filter(regex = featureRegex)
        for col in df:
            item = df.loc[df.index[0], col]
            if not negativeOnly or (negativeOnly and item<0):
                caller.algo.MyDebug(f' {col}: {item}')
        return
...
class MyStrategySettings():
    ...
    def UpdateSettings(self):
        ...
        for strategy in self.algo.myStrategyClassList:
            ...
            #strCode in Settings should be Lower Case
            if str(strCode).lower in df: strCode = str(strCode).lower
            if strCode in df:
                for row in range(df.shape[0]):
                    prop = df.index[row]
