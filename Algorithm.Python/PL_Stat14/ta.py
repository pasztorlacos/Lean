...
class MyDCHState():
    ...
    def __init__(self, caller, baseDCH, referenceDCH=None, name=None, attrType='dch'):
        self.debug = False 
    ...
    def Update(self, caller, bar):
        if self.attrType == 'dch':
            if (not self.baseDCH.IsReady) or (self.referenceDCH !=None and (not self.referenceDCH.IsReady)): return
        else:
            if not self.caller.IsReady(): return
        
        self.atr = self.caller.atr1.Current.Value if self.caller.atr1.Current.Value !=0 else 1
        
        if self.attrType == 'dch':
            baseDCH_Upper       = self.baseDCH.UpperBand.Current.Value
            baseDCH_Lower       = self.baseDCH.LowerBand.Current.Value
            referenceDCH_Upper  = self.referenceDCH.UpperBand.Current.Value if self.referenceDCH!=None else None
            referenceDCH_Lower  = self.referenceDCH.LowerBand.Current.Value if self.referenceDCH!=None else None
        else:
            base_rw = self.caller.bars_rw if self.baseDCH[0] == 'rw1' else self.caller.bars_rw_2
            if self.referenceDCH!=None: reference_rw = self.caller.bars_rw if self.referenceDCH[0] == 'rw1' else self.caller.bars_rw_2
            baseDCH_Upper       = max([bar.High for bar in base_rw][1:self.baseDCH[1]])
            baseDCH_Lower       = min([bar.Low  for bar in base_rw][1:self.baseDCH[1]])
            referenceDCH_Upper  = max([bar.High for bar in reference_rw][1:self.referenceDCH[1]]) if self.referenceDCH!=None else None
            referenceDCH_Lower  = min([bar.Low  for bar in reference_rw][1:self.referenceDCH[1]]) if self.referenceDCH!=None else None
        
        self.size = (baseDCH_Upper - baseDCH_Lower)/self.atr
        if self.referenceDCH!=None:
            self.sizeRef = (referenceDCH_Upper - referenceDCH_Lower)/self.atr
        else:
            self.sizeRef = 0
        
        if self.referenceDCH!=None and self.sizeRef != 0:
            #self.diffUpper = (referenceDCH_Upper - baseDCH_Upper)/self.atr/self.sizeRef
            if not self.CL.gabor_diffUpper:
                #Old version
                self.diffUpper = (referenceDCH_Upper - baseDCH_Upper)/self.atr/self.sizeRef
            else:
                self.diffUpper = (baseDCH_Upper - referenceDCH_Lower)/self.atr/self.sizeRef
            self.diffLower = (baseDCH_Lower - referenceDCH_Lower)/self.atr/self.sizeRef
        else:
            self.diffLower = 0
            self.diffUpper = 0
        self.priceFromUpper = (baseDCH_Upper - bar.Close)/self.atr
        self.priceFromLower = (bar.Close - baseDCH_Lower)/self.atr
            
        self.event = 0
        self.statusChange = 0
        if bar.High > baseDCH_Upper: 
            self.event = 1
            self.eventMove = bar.High - baseDCH_Upper
            if self.status == -1: 
                self.statusChange = 1
            self.status = 1
        if bar.Low < baseDCH_Lower: 
            self.event = -1
            self.eventMove = bar.Low - baseDCH_Lower
            if self.status == 1: 
                self.statusChange = -1
            self.status = -1
        ...
        if bar.Close > (baseDCH_Upper + self.CL.signalMarginATRLong*self.atr) and self.signalDisabledLong==0:
            self.signal = 1
            self.caller.signals += self.name_L
            self.signalDisabledLong = self.CL.freezeSignalLong+1
        elif bar.Close < (baseDCH_Lower - self.CL.signalMarginATRShort*self.atr) and self.signalDisabledShort==0:
            self.signal = -1
...
class MyMAState():
    ...
    def __init__(self, caller, baseMA, referenceMA=None, name=None, attrType='sma'):
        self.debug = False 
        self.CL = self.__class__
        self.caller = caller
        self.baseMA = baseMA
        ...
        self.caller.stateUpdateList.append(self)
        self.atr = 0
        self.name_L = f'L_MA_{name}-'
        self.name_S = f'S_MA_{name}-'
    ...
    def Update(self, caller, bar):
        if self.attrType == 'sma':
            if (not self.baseMA.IsReady) or (self.referenceMA!=None and (not self.referenceMA.IsReady)): return
        else:
            if not self.caller.IsReady(): return
        ...
        if self.attrType == 'sma':
            baseMA_Value =      self.baseMA.Current.Value
            referenceMA_Value = self.referenceMA.Current.Value if self.referenceMA!=None else None
        else:
            base_rw = self.caller.bars_rw if self.baseMA[0] == 'rw1' else self.caller.bars_rw_2
            if self.referenceMA!=None: reference_rw = self.caller.bars_rw if self.referenceMA[0] == 'rw1' else self.caller.bars_rw_2 
            baseMA_Value =      sum([bar.Close for bar in base_rw][0:self.baseMA[1]])/self.baseMA[1] 
            referenceMA_Value = sum([bar.Close for bar in reference_rw][0:self.referenceMA[1]])/self.referenceMA[1] if self.referenceMA!=None else None
        
        self.event = 0
        self.statusChange = 0
        if bar.Close > (baseMA_Value + self.CL.signalMarginATRLong*self.atr): 
            #Break
            if self.status == -1: 
                self.statusChange = 1
                self.breakMA = baseMA_Value
                self.lastHigh = baseMA_Value
            self.status = 1
        ...
        self.lastHighFromMA = (self.lastHigh-baseMA_Value)/self.atr
        self.lastLowFromBreak = (self.lastLow-self.breakMA)/self.atr
        self.lastLowFromMA = (self.lastLow-baseMA_Value)/self.atr
        ...
        if self.referenceMA!=None:
            self.diff = (baseMA_Value - referenceMA_Value)/self.atr
        else:
            self.diff = 0
        self.priceFromMA = (bar.Close - baseMA_Value)/self.atr
        self.priceFromBreak = (bar.Close - self.breakMA)/self.atr
        
        if self.statusChange == 1 and bar.Close > (baseMA_Value + self.CL.signalMarginATRLong*self.atr) and self.signalDisabledLong==0:
            self.signal = 1
            self.caller.signals += self.name_L
            self.signalDisabledLong = self.CL.freezeSignalLong+1
            self.lastSignalAge=0
        elif self.statusChange == -1 and bar.Close < (baseMA_Value - self.CL.signalMarginATRShort*self.atr) and self.signalDisabledShort==0:
            self.signal = -1
            self.caller.signals += self.name_S
            self.signalDisabledShort = self.CL.freezeSignalShort+1
            self.lastSignalAge=0
...
class MyVolatility():
    ...
    # Update method is mandatory!
    def Update(self, bar):
        #First UpdatrSet up benchmarkSymbol and benchmarkVol. During init benchmarksymbol is not added yet, so it has to be done at the forst update. 
        if self.benchmarkSymbol==None:
