...
class MyPositionManagerB:
    ...
    def LiquidateAlgo(self):
        ...
        for x in self.algo.Portfolio:
            if self.algo.Portfolio[x.Key].Quantity != 0:
                self.LiquidatePosition(x.Key, "AlgoLi", " --- Algo Liquidation")
    ...
    def LiquidatePosition(self, orderSymbol, orderTag, message):
        ...
        for tempTicket in self.algo.openMarketOrders:
            if tempTicket.Symbol == orderSymbol and (tempTicket.Status == OrderStatus.Submitted or tempTicket.Status == 6 or tempTicket.Status == OrderStatus.New):
                _CM += 1
        ...
        for tempTicket in self.algo.openStopMarketOrders:
            if tempTicket.Symbol == orderSymbol and (tempTicket.Status == OrderStatus.Submitted or tempTicket.Status == 6):
                _CS += 1
        ...
        #Cancelling Stops
        for tempTicket in self.algo.openStopMarketOrders:
            if tempTicket.Symbol == orderSymbol and (tempTicket.Status == OrderStatus.Submitted or tempTicket.Status == 6) and tempTicket.Status != OrderStatus.CancelPending:
        ...
        if abs(positionQuantity + _SM)>=quantityLimit:
            ...
            orderTag2 = "MLiq: " + str(entryPrice) +" ---_SP:"+str(_SP)+" _SS:"+str(_SS)+" _SL:"+str(_SL)+"_SM:"+str(_SM)+"_"+str(orderTag)+"_"+str(self.algo.Time)
