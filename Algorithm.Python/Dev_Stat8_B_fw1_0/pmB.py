...
class MyPositionManagerB:
    ...
    def SymbolOrderConsistency(self, orderSymbol):
        ...
        if _SS < 0:
            _UT = getattr(self.algo.mySymbolDict[orderSymbol].CL, "useTragetsLong", self.algo.mySymbolDict[orderSymbol].CL.targetPlacerLong!=None)
        else:
            _UT = getattr(self.algo.mySymbolDict[orderSymbol].CL, "useTragetsShort", self.algo.mySymbolDict[orderSymbol].CL.targetPlacerShort!=None)
