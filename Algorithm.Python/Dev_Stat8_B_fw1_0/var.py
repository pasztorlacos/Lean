    ...
    def UpdateExposure (self):
        ...
        for symbol in self.algo.mySymbolDict:
            ...
            if var.icnludeinTotalVaR and portfolioValue!=0:
                if _SS < 0:
                    useTargets = getattr(self.algo.mySymbolDict[symbol].CL, "useTragetsLong", self.algo.mySymbolDict[symbol].CL.targetPlacerLong!=None)
                else:
                    useTargets = getattr(self.algo.mySymbolDict[symbol].CL, "useTragetsShort", self.algo.mySymbolDict[symbol].CL.targetPlacerShort!=None)
