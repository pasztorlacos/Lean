...
class MyBPA():
    def __init__(self, caller, algo, symbol, name, period, atr, lookbackLong=1, lookbackShort=1, \
        ...
        self.symbol = symbol
        self.name = name
        self.name_L = f'L_BPA_{name}_-'
        self.name_S = f'S_BPA_{name}_-'
        self.period = period
        self.atr = atr
        self.lookbackLong = lookbackLong
        self.lookbackShort = lookbackShort
...
class MyGASF():
    ...
    #
    def Spectrum(self, x, components, useLog=True, distribution=True):
        n = x.size
        n_dft = n//2

        fft = np.fft.fft(x)
        fft_amp = np.abs(fft)[:n_dft]*1/n
        
        components = min(components, n_dft)
        windowSize = n_dft/components
    ...
    #   
    def FeatureExtractorDFT(self, n=None, components=None, featureType="Close", useLog=True, distribution=True):
        ...
        n = min(n, len(self.myBars)) if n!=None else len(self.myBars)
        if components==None : components = n
        ...
        Features = self.Spectrum(x, components, useLog=useLog, distribution=distribution)
        return Features